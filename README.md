...
它们在**规则匹配之前**就生效，模块无法覆盖：

**1. 设置 > 隧道 > 包括所有网络**

* **包括APNs** → **关**
* **包括蜂窝服务** → **关**（对应 VoLTE、Wi-Fi 通话、IMS、MMS、可视语音信箱）
* **包括本地网络** → **关**

最省事的做法是直接关闭「包括所有网络」总开关。

**2. 设置 > 代理 > 代理类型 → 选「HTTP」，不要选「None」**
...
2. 若确认相关，先只把**代理类型从 None 改成 HTTP**，不改规则，再测一次。
3. 仍不通，再关「包括所有网络」。
4. 仍不通，导入模块让 APNs 直连。
5. 只有在用港澳台 / 境外卡且确认该运营商发布了 ePDG 的前提下，才需要启用模块里的
   `tun-excluded-routes`。

每一步之间留足时间让 APNs 重新建连（拔插飞行模式可加速）。
